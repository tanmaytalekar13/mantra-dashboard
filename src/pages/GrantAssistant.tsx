...
export default function GrantAssistant() {
  ...
  const filteredGrants = useMemo(() => {
    return unifiedGrants.filter((g) => {
      const matchName =
        selectedGrantName === "All" || g.name === selectedGrantName;

      const matchMonth =
        selectedMonth === "All" || g.month === selectedMonth;
      ...
      <div className="flex h-screen items-center justify-center text-red-600">
        <div className="text-center">
          ...
          <p className="text-sm text-red-400 mt-2">
            Check that the grant CSV files are present in
        ...
        <>
          <div className="rounded-xl bg-white shadow p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              ...
              <SummaryItem
                label="Donor"
                value={selectedGrant.donor}
              />

              <SummaryItem
                label="Reporting Month"
                value={
                  selectedGrant.monthLabel ??
                  MONTH_LABELS[selectedGrant.month] ??
                  selectedGrant.month
                }
              />
